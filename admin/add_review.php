...
<?php
				...
				$Fulldatetime_review =  $Datetime_review." ".$timehou.":".$timemin.":".date("s"); //รวม Format วันเวลา 2018-08-29 18:27:06
					...
					//ดึงลำดับล่าสุดแล้ว +1 เข้าไป
					$sql_maxreview="select MAX(No_review) from tbl_review";
					$result_maxreview=mysqli_query($conn,$sql_maxreview);
					$rs_maxreview=mysqli_fetch_array($result_maxreview);
					$Maxnoreview =  $rs_maxreview[0]+1;
					
					$sql_addreview = "INSERT INTO tbl_review
					VALUES (null,'".$Maxnoreview."','','".$Fulldatetime_review."','".$Fullname_review."','".$Level_review."','".$Message_review."','Show')";
					$result=mysqli_query($conn,$sql_addreview);
	...
	$ext = strtolower(end(explode('.',$Img_gallerry_name)));
		  ...
		  /* จบ กรณีเงื่อนไขผ่านหมด จะเข้ามาทำในส่วนนี้*/
					...
					echo"<script>alert('เพิ่มข้อมูลรีวิวเรียบร้อยแล้ว')</script>";
					echo"<script>window.location='set_config.php'</script>";
					exit();
?>
